...
<div class="space-y-6">
    ...
    @endif
        <form wire:submit.prevent="submitStoreChanges" enctype="multipart/form-data">
            ...
            <div>
                <label for="pendingStoreName">Propose New Store Name:</label>
                <input type="text" wire:model.defer="pendingStoreName" class="border p-2 w-full"/>

                @error('pendingStoreName')
            ...
            @if ($pendingLogoExists)
                <div>
                    <label>Pending Logo (awaiting approval):</label>
                    <img src="{{ asset(tenant()->id . '/assets/img/pending/new_logo.png') }}"
                         class="w-24 h-24 border mt-2 object-contain">
                </div>
            @endif

            <div>
                <label for="logo">Upload New Logo (smaller than 200x200 pixels):</label>
                <input type="file" wire:model="logo" class="mt-2">
